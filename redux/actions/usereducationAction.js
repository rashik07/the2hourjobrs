...
export const viewTraining = (data) => async (dispatch) => {
  try{    
    const response = await backend.get(`/v1/user/training/?user=${store.getState().auth.id}`, getConfig());
...
export const viewQualification = (data) => async (dispatch) => {
  try{    
    const response = await backend.get(`/v1/user/professional-qualification/?user=${store.getState().auth.id}`, getConfig());
